...
RSpec.describe "Users API", type: :request do
  ...
  describe "POST /v1/api/user/email" do
    ...
    session.save
    let(:valid_attributes) { { email: "alex@leaguefund.xyz", session_id: session.session_id } }
  ...
  describe "POST /v1/api/user/validate-email" do
    ...
    session.user_id = user.id
    session.save
    let(:valid_attributes) { { email: "alex@leaguefund.xyz", token: "123-456", session_id: session.session_id } }
